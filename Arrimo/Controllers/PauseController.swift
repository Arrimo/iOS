...
class PauseController: UIViewController, CLLocationManagerDelegate {
    ...
    private func sendDictionary() {
        if locationManager.location?.coordinate == nil {
            ...
            self.sendJSON(action: "resume", long: self.locationManager.location!.coordinate.longitude, lat: self.locationManager.location!.coordinate.latitude, user: nil, caretaker: RunningInfo.shared.employee!.id!, tasks: nil)
            completion()
        }
    }
